...
_asm_hrb_api:
		...
		PUSH	ES
		PUSHAD		; 用于保存的PUSH
		PUSHAD		; 为了传递给hrb_api而PUSH
		MOV		AX,SS
		MOV		DS,AX		; 把OS用的段也放入DS和ES
		MOV		ES,AX
		CALL	_hrb_api
		CMP		EAX,0		; 如果EAX不为0，就进行应用结束处理
		JNE		end_app
		ADD		ESP,32
		POPAD
		POP		ES
...
_start_app:		; void start_app(int eip, int cs, int esp, int ds, int *tss_esp0);
		...
		MOV		FS,BX
		MOV		GS,BX
;	下面是为了用RETF跳转到应用而进行的栈调整
		OR		ECX,3			; 对应用用的段号进行OR 3
		OR		EBX,3			; 对应用用的段号进行OR 3
		PUSH	EBX				; 应用的SS
		PUSH	EDX				; 应用的ESP
		PUSH	ECX				; 应用的CS
		PUSH	EAX				; 应用的EIP
