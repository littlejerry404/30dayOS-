 ...
 * 2003/09/27 | 为PICTURE0.BIN(DLL)进行改造 ( by Kumin )
 * 2003/09/28 | 各种bug修复&一些优化 ( by H.Kawai )
 *
 */
...
typedef struct
{
	...
	int base_img[64][64]; // 基底图像 ( [横向频率uπ][纵向频率vπ][横向相位(M/8)][纵向相位(N/8)]
...
//	if (jpeg->width == 0)
//		return 8;
	/* 在decode0中是在info之后被调用的，所以没有这种情况 */
...
unsigned short get_bits(JPEG *jpeg, int bit)
{
	...
	while (remain <= 16) {
		...
		c = *jpeg->fp++;
		if (c == 0xff) { // 为防止标记错误，FF被转义成 FF 00
			if (jpeg->fp >= jpeg->fp1) {
				ret = 0;
				goto fin;
			}
	...
	while (size > 0) {
		...
		if (j > jpeg->n_dqt)
 		...
 		if (c & 0xf8) {
			// 16 bit DQT
			for (i = 0; i < 64; i++) {
				jpeg->dqt[j][i] = jpeg->fp[0]; /* 低位被读后丢弃 */
				jpeg->fp += 2;
			}
			size += -64 * 2;
		} else {
...
int jpeg_dht(JPEG *jpeg)
{
	...
	while (len > 0) {
		...
		tc = (val >> 4) & 0x0f; // 表类别(DC/AC成分选择器)
		th =  val       & 0x0f; // 表头(第几个平面)
