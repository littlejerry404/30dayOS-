...
int tek_getsize(unsigned char *p)
{
	...
	return size;
}	  /* （注）memcmp是strncmp的同类，即使字符串中间有0也会一直比较到指定的15个字符为止的函数 */
...
static int tek_lzrestore_tek5(int srcsiz, UCHAR *src, int outsiz, UCHAR *outbuf)
{
	...
	wrksiz = 0x180 * sizeof (UINT32) + (0x840 + (0x300 << (lc + lp))) * sizeof (tek_TPRB); /* 最低15KB，如果lc+lp=3，则为36KB */
