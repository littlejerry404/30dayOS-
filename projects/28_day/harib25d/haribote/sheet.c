...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
	...
	for (h = h0; h <= ctl->top; h++) {
		sht = ctl->sheets[h];
		sid = sht - ctl->sheets0; /* 对地址做减法，把它作为图层编号使用 */
		buf = sht->buf;
		bx0 = vx0 - sht->vx0;
		by0 = vy0 - sht->vy0;
		bx1 = vx1 - sht->vx0;
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0, int h1)
{
	...
	for (h = h0; h <= h1; h++) {
		...
		if ((sht->vx0 & 3) == 0) {
			/* 4字节型 */
			i  = (bx0 + 3) / 4; /* bx0除以4（余数向上取整） */
			i1 =  bx1      / 4; /* bx1除以4（余数向下取整） */
			i1 = i1 - i;
			sid4 = sid | sid << 8 | sid << 16 | sid << 24;
			for (by = by0; by < by1; by++) {
				vy = sht->vy0 + by;
				for (bx = bx0; bx < bx1 && (bx & 3) != 0; bx++) {	/* 前面的零头每次1字节 */
					vx = sht->vx0 + bx;
				...
				for (bx += i1 * 4; bx < bx1; bx++) {				/* 后面的零头每次1字节 */
					vx = sht->vx0 + bx;
					if (map[vy * ctl->xsize + vx] == sid) {
						vram[vy * ctl->xsize + vx] = buf[by * sht->bxsize + bx];
					}
...
void sheet_updown(struct SHEET *sht, int height)
{
	...
	if (old > height) {	/* 比以前更低 */
		if (height >= 0) {
			...
			ctl->top--; /* 因为正在显示的图层减少一个，所以最上面的高度减少 */
			sheet_refreshmap(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize, 0);
			sheet_refreshsub(ctl, sht->vx0, sht->vy0, sht->vx0 + sht->bxsize, sht->vy0 + sht->bysize, 0, old - 1);
		}
	} else if (old < height) {	/* 比以前更高 */
		if (old >= 0) {
			/* 把中间的压下去 */
			for (h = old; h < height; h++) {
				ctl->sheets[h] = ctl->sheets[h + 1];
			...
			ctl->sheets[height] = sht;
			ctl->top++; /* 因为正在显示的图层增加一个，所以最上面的高度增加 */
		}
...
{
	if (sht->height >= 0) { /* 如果正在显示，就根据新图层的信息重新绘制画面 */
...
{
	if (sht->height >= 0) {
		sheet_updown(sht, -1); /* 如果正在显示就先设为不显示 */
	}
	sht->flags = 0; /* 未使用标记 */
	return;
}
